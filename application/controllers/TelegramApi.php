	...
	public function start_post()
    {
           ...
           if(strtolower($userInput) == 'start' || strtolower($userInput) == '/start'){
			...
			if($existing_user['stage_position'] == 6){
				...
				$msg = 'Kindly deposit the exact sum of '.number_format(($transaction_details['charges'] + $transaction_details['amount']), 2).'USDT asset into the wallet address below:'.chr(10).chr(10).'<code>Wallet Address:: 0x9d8290f731D38CF2feF112Ac74F1dDa69509cf18</code>'.chr(10).chr(10).chr(10).'On successful payment into KwikExchage wallet, Kindly provide the <code>Transaction ID</code> as proof of payment on Transfer Status Menu'.chr(10).chr(10).'NETWORK:: BINANCE/ETHEREUM SMART CHAIN'.chr(10).chr(10).chr(10).' Your KwikExchange Transaction code for this transfer is: <code>'.chr(10).$transaction_details['transaction_code'].'</code>'.chr(10).'COPY IT!!'.chr(10).chr(10).chr(10).'Click /START to go to Dashboard';
